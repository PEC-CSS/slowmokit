...
pub fn write_files(model_name: String, model_type: String) {
    let methods_folder = "include/slowmokit";
    ...
    fs::create_dir_all(format!("{tests_folder}/{model_type}"))
        .expect("An Error occured while creating tests directory.");

    // source code files
    let _easy_import_file =
        fs::File::create(format!("{methods_folder}/{model_type}/{model_name}.hpp"))
    ...
    let _imple_file = fs::File::create(format!(
        "{methods_folder}/{model_type}/{model_name}/{model_name}_impl.hpp"
    ))
    .expect("Error encountered while creating file!");
    ...
    // examples files
    let _examples_file =
        fs::File::create(format!("{examples_folder}/{model_type}/{model_name}.cpp"))
            .expect("Error encountered while creating file!");

    // tests files
    let _tests_file = fs::File::create(format!("{tests_folder}/{model_type}/test_{model_name}.cpp"))
        .expect("Error encountered while creating file!");
    ...
    write_source::write_easy_import(
        _easy_import_file,
        model_name.to_owned(),
        model_type.to_owned(),
    );
