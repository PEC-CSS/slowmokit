    ...
    double testSize, double trainSize, bool shuffle)
{
    ...
    auto xData = std::vector<std::pair<T, G>>(std::begin(input), std::begin(input) + trainDataSize);
    auto yData = std::vector<std::pair<T, G>>(std::begin(input) + trainDataSize, std::end(input));
