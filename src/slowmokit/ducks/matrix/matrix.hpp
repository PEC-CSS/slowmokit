...
template<class T = int> class Matrix
{
  ...
  void updateShape()
  {
    assert(std::size(mat) > 0 and std::size(mat[0]) > 0 and
           "Cannot have empty dimension");
    n = std::size(mat);
    m = std::size(mat[0]);
  }
   ...
   * @throws: std::out_of_range exception if any dimension is non-positive
   * @throws: std::invalid_argument exception if dimensions of two matrices are
  ...
  /**
   * @brief overloading -= operator for subtracting another matrix from existing
   * matrix
