...
RealType random(RealType L, RealType R)
{
    ...
    if (L > R)
        throw std::range_error("Start element of range must be less than equal to End element of range.");
...
IntType randint(IntType L, IntType R)
{
    ...
    if (!std::is_integral<IntType>() and !std::is_unsigned<IntType>())
        throw std::domain_error("Both parameter must be signed or unsigned integer type only.");
