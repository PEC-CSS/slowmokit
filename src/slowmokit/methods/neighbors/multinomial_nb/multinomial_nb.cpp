...
{ // Conditional-Probabilty P(x=f1 / y=class)
  ...
  for (int i = 0; i < yTrain.size(); i++)
  {
    ...
    {                // Check if label of feature is same as output looking for
      ...
      {
        numerator++; // numerator increase only when that feature consists of
                     // value same as value in testing feature
      }
    }
  }
  return (numerator + 1) /
                                  ...
                                  std::vector<T> xTest)
{
  ...
  { // iterating over all labels (P(y=y1,y2,y3....))
    ...
    for (int i = 0; i < n_features; i++)
    {
      ...
      likelihood += log10(cond); // Log10 of computed conditionl probabilty =>
                                 // Log10(P(x=x1 / y=label))
    }
  ...
  { // Iterating over all labels computed values to check if any negative value
