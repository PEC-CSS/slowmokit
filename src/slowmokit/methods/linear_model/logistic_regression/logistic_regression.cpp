...
{ // Softmax function
	...
	{ // Iterating over vector to compute sum of all exponential values in
	  // vector
		exps[i] = exp(x[i]);
		sum += exps[i];
	}
...
{                                 // PERFORMS MULTICLASS LOGISTIC REGRESSION
	     ...
	     i++) // loop running for number of epochs specified
	{
		...
		std::vector<int> prm(
		    n); // Making vector of size 'n' so that we can
		                     ...
		                     // the while loop
		{
			...
			std::vector<double> xiHat =
			    x[frnt]; // now we take random value of x according to the frnt
			             // value obtained!
			std::vector<int> yi =
			    y[frnt]; //   Gives y value corresponding to xiHat
			std::vector<double> zi(
			    numClasses,
			    0.0); // Initialization of zi vector which contains the
			          // predicted value of xiHat(one hot encoded)
			          //   Formula for z[i] is written below

			// z[i] = weight[ouput class][j-th weight] * x[i-th row][j-th
			// feature]
			...
			{ // Iterating over classes of output
				...
				{ // Iterating over x[i]
					zi[j] += beta[j][k] *
					    xiHat[k]; // Here we are performing  |  z[i] =
					              // weight[ouput class][j-th weight] * x[i-th
					              // row][j-th feature]
				}
			}
			zi =
			    softmax(zi); // Now we are computing here the softmax of all one
			                 // hot encoded values obtained for particular xiHat
			...
			for (int j = 0; j < numClasses; j++) // iterating over all classes
			{
				...
				{
					gradLi[j][k] +=
					    (zi[j] - yi[j]) * xiHat[k]; // here we are computing
					                                // gradient descent of batch
				}
			}

			countCompleted++; // updating countCompleted means we added loss of
			                  // this many values
			...
			prm.pop_back(); // deleting prm last value to take next random value
			                // for xiHat
		}
	}
