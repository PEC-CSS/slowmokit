...
/**
 * @file linear_model/linear_regression/linear_regression_impl.hpp
 *
 * Implementation of linear_regression algorithm.
 */

#ifndef SLOWMOKIT_LINEAR_REGRESSION_IMPL_HPP
#define SLOWMOKIT_LINEAR_REGRESSION_IMPL_HPP
...
namespace slkt
{
	...
	template <class T>
	void LinearRegression<T>::fit(std::vector<std::vector<T>> x,
	                              std::vector<T> y)
	{
		if (x.empty() || y.empty())
		{
		...
		for (int epoch = 0; epoch < this->epochs; epoch++)
		{
			...
			for (int example = 0; example < trainExampleSize; example++)
			{
				coefficients[0] -= learningRate *
				    (yPred[example] - y[example]) / trainExampleSize;
			}
			for (int feature = 0; feature < featureSize; feature++)
			{
				for (int example = 0; example < trainExampleSize; example++)
...
#endif // SLOWMOKIT_LINEAR_REGRESSION_IMPL_HPP
